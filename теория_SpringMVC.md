...
### Продвинутая валидация (Spring Validator) ( проект SpringMVCApp7SpringValidator)
Аннотации, которые навешиваются на оторажаемую сущность, позводяют сделать красивый вывод с помощью объекта класса **Binding Result**, в случае, если в вводимую форму в HTML-представлении введено некорректное значение. Если во время ввода ошибка на стороне БД (сработала одна из **constraint**)),то вылетает ошибка сервера с некрасивым выводом. Для этих целей есть Spring Validator.
...
### Валидация паттернов (проект SpringMVCApp7SpringValidator)
Аннотация **@Pattern** - позволяет проверить что строка соответствует определенному паттерну, этот паттерн задается с помощью регулярных выражений.  
```
